...
func main() {
	...
	port    := flag.String("port", "7013", "Port to listen on")
...
func (s *Server) acceptLoop() {
	...
	for {
		...
		if s.stats.ActiveConns.Load() >=  int64(s.config.MaxConns) {
			...
			fmt.Fprintf(conn, "REQUEST REJECTED MAX CONNECTION REACHED")
			conn.Close()
			log.Printf("[SERVER] Max Connections reached")
			continue
		}
...
func (s *Server) handleConnection(conn net.Conn) {
	...
	for{
		...
		_, err2 := fmt.Fprintf(conn, "RESPONSE OK %s\n", time.Now().Format(time.RFC3339))
		if err2 != nil {
			fmt.Println("Failed to send",err2.Error())
