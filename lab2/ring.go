...
func (n *Node) findSuccessor(id uint8) (NodeInfo, error) {
	...
	if inRange(id, n.info.ID, succ.ID) {
		return succ, nil
	}

	next := n.closestPrecedingFinger(id)

	if next.Addr == n.info.Addr {
		return n.info, nil
	}
	...
	err := callRPC(
		next.Addr,
		"ChordRPC.FindSuccessor",
		&FindSuccessorArgs{ID: id},
		&reply,
	)
...
func (n *Node) closestPrecedingFinger(id uint8) NodeInfo {
	...
	for i := M - 1; i >= 0; i-- {
		...
		if inRange(finger.ID, n.info.ID, id) {
			return finger
		}
	}
...
func (n *Node) join(existingAddr string) error {
	...
	err := callRPC(
		existingAddr,
		"ChordRPC.FindSuccessor",
		&FindSuccessorArgs{ID: n.info.ID},
		&reply,
	)
...
func (n *Node) stabilize() {
	...
	if predReply.Node != nil &&
		inRange(predReply.Node.ID, n.info.ID, succ.ID) {

		n.mu.Lock()
		n.fingers[0] = *predReply.Node
		n.mu.Unlock()
	}
